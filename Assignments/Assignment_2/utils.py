...
def xor(bit_string1, bit_string2):
    '''
        Performs XOR on two bit strings.
    '''
    return ''.join(str(int(b1) ^ int(b2)) for b1, b2 in zip(bit_string1, bit_string2))

def rotate_left(input_bit_string, shift_amount):
    return input_bit_string[shift_amount:] + input_bit_string[:shift_amount]
...
def bit_array_to_hex(bit_array):
    return hex(int(bit_array, 2))[2:].upper()
